...
import { getUserRole } from '../../../lib/graphql/query.js'
...
export const authOptions: NextAuthOptions = {
  ...
  callbacks: {
    ...
    async session({session, token, user }) {
      if(user) {
        const {data:{getUser}} = await client.query(getUserRole,{userID:user.id}).toPromise();
        if(getUser !== null) {
          console.log('role to add ',getUser.role);
          session.role = getUser.role; // Add role value to user object so it is passed along with session
        }
      }
      return session;
    }
